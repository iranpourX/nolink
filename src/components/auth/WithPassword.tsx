...
export const WithPassword = () => {
    ...
    const {loading, withPassword, backToOtp} = useOtp()

    const {
        handleSubmit,
        register,
        formState: {errors},
    } = useForm<withPassword>()

    const onSubmitPassword: SubmitHandler<withPassword> = async (value) => {
        await withPassword(value)
    }

    return (
        <>
            <div
                className="mb-4 sm:mb-8 flex flex-col justify-center items-center">
                <h1 className="mb-2 font-semibold text-gray-800 text-xl">
                    Enter your Password
                </h1>
            </div>

            <form onSubmit={handleSubmit(onSubmitPassword)} className="flex flex-col">
